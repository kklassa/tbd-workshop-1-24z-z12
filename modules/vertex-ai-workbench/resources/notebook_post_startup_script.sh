...
export PYSPARK_SUBMIT_ARGS="--packages com.databricks:spark-xml_2.12:0.17.0 --conf spark.executor.instances=3 --conf spark.executor.memory=2g --conf spark.sql.legacy.timeParserPolicy=LEGACY --jars /home/jupyter/gcs-connector-hadoop3-${GCS_CONNECTOR_VERSION}-shaded.jar --master yarn --conf spark.hadoop.fs.gs.impl=com.google.cloud.hadoop.fs.gcs.GoogleHadoopFileSystem --conf spark.hadoop.fs.AbstractFileSystem.gs.impl=com.google.cloud.hadoop.fs.gcs.GoogleHadoopFS --conf spark.driver.port=16384 --conf spark.driver.blockManager.port=16385 --conf spark.driver.bindAddress=${VERTEX_CONTAINER_IP} --conf spark.driver.host=${VERTEX_CONTAINER_HOSTNAME} --conf spark.network.timeout=600s --conf spark.rpc.askTimeout=600s pyspark-shell"
